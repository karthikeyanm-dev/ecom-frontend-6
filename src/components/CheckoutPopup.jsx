    ...
    useEffect(() => {
        ...
        const fetchImages = async () => {
            ...
            for (const item of cartItems) {
                try {
                    const res = await API.get(`/product/${item.id}/image`, { responseType: "blob" });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative z-10 w-full max-w-lg mx-4 bg-[#161b27] border border-[#1e2a3a] rounded-xl p-5">
                ...
                <div className="flex justify-between items-center border-b border-[#1e2a3a] pb-3 mb-4">
                    ...
                    <button onClick={handleClose} className="text-slate-500 hover:text-red-500 text-sm">✕</button>
                </div>

                {/* Items */}
                <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                    {cartItems.map((item) => (
                        <div key={item.id} className="flex items-center gap-3 border-b border-[#1e2a3a] pb-3">
                            <img
                ...
                <div className="flex justify-between items-center border-t border-[#1e2a3a] mt-4 pt-4">
                    ...
                    <span className="text-xl font-extrabold text-blue-400">₹{totalPrice.toLocaleString()}</span>
                </div>
